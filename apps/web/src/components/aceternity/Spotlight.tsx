...
// Pure SVG + CSS; with prefers-reduced-motion the beam is rendered static (no animation).
export function Spotlight({ className, fill = 'white' }: { className?: string; fill?: string }) {
  const reduce = useReducedMotion();
  return (
    <svg
      aria-hidden
      className={cn('pointer-events-none absolute z-[1] h-[169%] w-[138%] opacity-0 lg:w-[84%]', reduce ? 'opacity-100' : 'animate-spotlight', className)}
